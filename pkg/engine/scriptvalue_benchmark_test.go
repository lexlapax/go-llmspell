...
func BenchmarkTypeChecking(b *testing.B) {
	...
	b.Run("ScriptValue_TypeCheck", func(b *testing.B) {
		...
		for i := 0; i < 100; i++ {
			values = append(values, NewStringValue("test string"))
		}
...
func BenchmarkMethodExecution(b *testing.B) {
	...
	executeNew := func(args []ScriptValue) (ScriptValue, error) {
		if len(args) < 2 {
			return nil, nil
		}
		if args[0].Type() != TypeString || args[1].Type() != TypeNumber {
			return nil, nil
		}
		prompt := args[0].(StringValue).Value()
		maxTokens := args[1].(NumberValue).Value()
		return NewObjectValue(map[string]ScriptValue{
			"prompt": NewStringValue(prompt),
			"tokens": NewNumberValue(maxTokens),
		}), nil
	}
	...
	b.Run("ScriptValue_Method", func(b *testing.B) {
		args := []ScriptValue{NewStringValue("test prompt"), NewNumberValue(100)}
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			executeNew(args)
		}
...
func BenchmarkComplexObjectConversion(b *testing.B) {
	...
	b.Run("ScriptValue_Conversion", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			...
			sv := ConvertToScriptValue(complexData)
			// Convert back
			_ = sv.ToGo()
		}
	})
...
func BenchmarkArrayOperations(b *testing.B) {
	...
	b.Run("ScriptValue_Array", func(b *testing.B) {
		elements := make([]ScriptValue, size)
		for i := 0; i < size; i++ {
			elements[i] = NewNumberValue(float64(i))
		}
		arr := NewArrayValue(elements)
...
func BenchmarkMemoryAllocation(b *testing.B) {
	...
	b.Run("ScriptValue_Allocation", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			values := make([]ScriptValue, 0, 10)
			values = append(values, NewStringValue("string"))
			values = append(values, NewNumberValue(42))
			values = append(values, NewBoolValue(true))
			values = append(values, NewObjectValue(map[string]ScriptValue{"key": NewStringValue("value")}))
			values = append(values, NewArrayValue([]ScriptValue{NewNumberValue(1), NewNumberValue(2), NewNumberValue(3)}))
		}
	})
}
...
func BenchmarkErrorHandling(b *testing.B) {
	...
	b.Run("ScriptValue_SafeCheck", func(b *testing.B) {
		values := []ScriptValue{NewStringValue("string"), NewNumberValue(42), NewBoolValue(true)}
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			for _, v := range values {
...
func BenchmarkAPIResponseProcessing(b *testing.B) {
	...
	b.Run("ScriptValue_Processing", func(b *testing.B) {
		sv := ConvertToScriptValue(apiResponse)
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			...
			if choices, ok := fields["choices"]; ok && choices.Type() == TypeArray {
				choicesArr := choices.(ArrayValue).Elements()
				if len(choicesArr) > 0 && choicesArr[0].Type() == TypeObject {
					firstChoice := choicesArr[0].(ObjectValue).Fields()
					if text, ok := firstChoice["text"]; ok && text.Type() == TypeString {
						_ = text.(StringValue).Value()
					}
				}
			}
			
			if usage, ok := fields["usage"]; ok && usage.Type() == TypeObject {
				usageFields := usage.(ObjectValue).Fields()
				if tokens, ok := usageFields["total_tokens"]; ok && tokens.Type() == TypeNumber {
