...
// Each shell has different completion mechanisms and syntax.
type Shell string
...
// It maintains the command structure and generates appropriate
// completion scripts for different shell environments.
...
// It can have subcommands, flags, and a description for
// help text and completion hints.
type Command struct {
	Name        string
	Description string
	Subcommands []Command
...
// It includes both long and short forms, value requirements,
// and possible enum values for completion.
type Flag struct {
	Long        string
	Short       string
	Description string
...
// It initializes empty command and flag lists for the specified program.
...
// Commands are used to generate appropriate completion suggestions.
func (g *CompletionGenerator) AddCommand(cmd Command) {
	g.Commands = append(g.Commands, cmd)
}

// AddGlobalFlag adds a global flag.
// Global flags are available for all commands and subcommands.
...
// It returns the appropriate completion script as a string,
// or an error if the shell is not supported.
...
// It accepts common shell names and aliases, returning
// the corresponding Shell type or an error.
...
// This can be used to display available options to users.
func GetSupportedShells() []Shell {
	return []Shell{Bash, Zsh, Fish, PowerShell, Sh}
}

// generateBash generates Bash completion script.
// It uses Bash's complete builtin with a custom completion function.
...
// It uses Zsh's sophisticated completion system with _arguments
// for rich command and flag completion.
...
// It uses Fish's complete builtin with declarative syntax
// for command and flag registration.
...
// It uses Register-ArgumentCompleter for native command completion
...
// Since POSIX sh doesn't support programmable completion,
// this generates a help reference instead.
